...
app.Time.OnUpdate = function()
    ...
    local updatedTime = "";
    if c.twentyFourHour then
        updatedTime = ("%s%s%s"):format(app.Time.hour, app.colorString(":"), app.Time.minute);
    else
        updatedTime = ("%s%s%s %s"):format(app.Time.hour, app.colorString(":"), app.Time.minute, app.colorString(app.Time.ampm));
    end
